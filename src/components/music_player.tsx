...
export default function MusicPlayer() {
  ...
  const handlePreviousTrack = () => {
    if (!playerInfo.currentTrack && playerInfo.currentTrack !== 0) return;
    if (!setPlayerInfo || !playerInfo) return;
    if (playerInfo.currentTrack === 0) {
      setPlayerInfo({
        ...playerInfo,
        currentTrack: playerInfo.playlist.length - 1,
      });
    } else {
      setPlayerInfo({
        ...playerInfo,
        currentTrack: playerInfo.currentTrack - 1,
      });
    }
  };
  ...
  const handleNextTrack = () => {
    if (!playerInfo.currentTrack && playerInfo.currentTrack !== 0) return;
    if (!setPlayerInfo || !playerInfo) return;
    if (playerInfo.currentTrack === playerInfo.playlist.length - 1) {
      setPlayerInfo({
        ...playerInfo,
        currentTrack: 0,
      });
